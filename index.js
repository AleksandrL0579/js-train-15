...
function estimateInvestment(principal, interestRate, years) {
  if (
    ...
    console.error("Усі аргументи повинні бути числами.");
    return null;
  }
  const a = principal * Math.pow(1 + interestRate, years);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  if (!Array.isArray(products)) {
    console.error("Input 'products' is not an array.");
    return null;
  }
  if (typeof maxPrice !== "number") {
    console.error("Input 'maxPrice' is not a number.");
    return null;
  }
  const totalPrice = products.reduce((accumulator, product) => {
    return accumulator + product.price;
  }, 0);
  const b = Math.fround(totalPrice);
  const c = Math.fround(maxPrice);
  return b > c;
